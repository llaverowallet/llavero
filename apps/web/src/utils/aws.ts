...
export async function getParameterValue(name: string): Promise<string> {
    const ssmClient = new SSMClient({ region: process.env.REGION }); // Replace REGION with your AWS region
...
export async function updateUserPoolClientCallbackUrl(clientId: string, userPoolId: string,callbackUrl: string): Promise<void> {
    const cognitoClient = new CognitoIdentityProviderClient({ region: process.env.REGION }); // Replace REGION with your AWS region
