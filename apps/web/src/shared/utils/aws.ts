...
import { CognitoIdentityProviderClient, DescribeUserPoolCommand, UpdateUserPoolClientCommand, UpdateUserPoolCommand, UpdateUserPoolCommandInput } from '@aws-sdk/client-cognito-identity-provider';
...
export async function getUserPoolData(userPoolId: string) {
    try {
        const client = new CognitoIdentityProviderClient({ region: process.env.REGION }); // replace REGION with your AWS region
