...
export async function main(event: { params: ICloudWalletInitParams }) {
    ...
    const cognitoUsername = await createUser(event.params.cognitoPoolId, event.params.config.username, event.params.config.password);
...
async function createUser(cognitoPoolId: string, username:string, password:string, phone:string) {
