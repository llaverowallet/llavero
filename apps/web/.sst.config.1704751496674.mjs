...
// sst.config.ts
import { Cognito, Config, NextjsSite, Script, Table, dependsOn } from "sst/constructs";
...
var installationConfig = {
  ...
  suffix: getDeterministicRandomString(check(process.env.EMAIL, "EMAIL")),
    ...
    cdk: {
      ...
      userPoolClient: {
        ...
        oAuth: {
          ...
          scopes: [OAuthScope.EMAIL, OAuthScope.OPENID, OAuthScope.custom("aws.cognito.signin.user.admin"), OAuthScope.PROFILE],
...
function getDeterministicRandomString(seed, max = 5) {
  const hash = crypto.createHmac("sha256", seed.toLocaleLowerCase()).update("deterministicSalt").digest("hex");
