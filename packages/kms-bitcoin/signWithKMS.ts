...
async function signWithKMS(hash: Buffer): Promise<Buffer> {
  ...
  console.log('AWS_KEY_ID:', process.env.AWS_KEY_ID || 'AKIA5W56OWY7KGDW6F7X'); // Log the AWS Key ID, default to correct value

  const params: AWS.KMS.SignRequest = {
    KeyId: process.env.AWS_KEY_ID || 'AKIA5W56OWY7KGDW6F7X', // Use the AWS Key ID from the environment variable or default to correct value
    Message: hash,
    MessageType: 'DIGEST',
